<template>
  <v-app>
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>เช็คอิน</v-toolbar-title>
...
export default {
  ...
  async created () {
    axios.get('http://localhost:7000/api/place/get-place').then(resp => {
      let number = 1
  ...
  methods: {
    ...
    async deleteItemConfirm () {
      await axios.post('http://localhost:7000/api/place/delete-place', {
    ...
    async saveAndUpdate () {
      ...
      if (this.editedItem.id === '') {
        await axios.post('http://localhost:7000/api/place/createAndUpdate-place', {
